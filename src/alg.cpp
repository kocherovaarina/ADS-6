...
class TPQueue {
 ...
 public:
    explicit TPQueue(Comparator c = Comparator()) : head(nullptr), comp(c) {}
    ...
    TPQueue(const TPQueue& other) : head(nullptr), comp(other.comp) {
    ...
    TPQueue(TPQueue&& other) noexcept : head(other.head), comp(std::move(other.comp)) {
        other.head = nullptr;
    }
